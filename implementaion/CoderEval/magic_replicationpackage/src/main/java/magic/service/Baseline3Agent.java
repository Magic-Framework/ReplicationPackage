...
public class Baseline3Agent {

    interface Baseline3Assistant {
        @UserMessage("{{userMessage}}")
        String chat(@V("userMessage") String userMessage);
    }

    private final Baseline3Assistant assistant;

    public Baseline3Agent() {
        ChatModel model = LLMConfig.getQwenModel();
        this.assistant = AiServices.builder(Baseline3Assistant.class)
                .chatModel(model)
    ...
    public String fixJavaIssue(String code, List<CodeViolation> codeViolations, int stateLine) {
        ...
        for (CodeViolation codeViolation : codeViolations) {
            sb.append("Solution.java:").append(codeViolation.getStartLine() - stateLine + 1).append(": ");
    ...
    public String fixPythonIssue(String code, List<CodeViolation> codeViolations, int stateLine) {
        PromptTemplate template = PromptTemplate.from(
                """
                        You are an assistant. Here is a piece of code:
                        ```python
        ...
        StringBuilder sb = new StringBuilder();
        for (CodeViolation codeViolation : codeViolations) {
            sb.append("Solution.py:").append(codeViolation.getStartLine() - stateLine + 1).append(": ");
            sb.append(codeViolation.getRuleName()).append(": ");
            sb.append(codeViolation.getDescription()).append("\n");
        }
        Map<String, Object> variables = Map.of("code", code, "issues", sb.toString());
